...
async fn run_test_compact_db() {
    let client = get_client();
    let vote_result = client.operations().compact_db().await;
...
async fn run_test_clear_cache() {
    let client = get_client();
    let vote_result = client.operations().clear_cache().await;
...
async fn run_test_toggle_slow_request_log() {
    let client = get_client();
    let vote_result = client.operations().toggle_slow_request_log(1000).await;
...
mod tokio_test {
    ...
    async fn test_compact_db() {
        run_test_compact_db().await;
    }

    #[tokio::test]
    async fn test_clear_cache() {
        run_test_clear_cache().await;
    }

    #[tokio::test]
    async fn test_toggle_slow_request_log() {
        run_test_toggle_slow_request_log().await;
    }
}
